...
starting at `--directory`. The WorkBuddy and Codex examples set uv's cache to
the project-local ignored `.uv-cache` directory so sandboxed clients do not
need write access to the user-level uv cache.
...
- WorkBuddy or Codex reports `MCP error -32000: Connection closed`: confirm its command
  includes `--cache-dir <repository>/mcp_servers/code_review/.uv-cache` before
  `--directory`, use forward slashes in Windows paths, then restart the client
  and open a new task.
